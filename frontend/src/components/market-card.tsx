...
export function MarketCard({
  ...
  useEffect(() => {
    if (marketData) {
      ...
      if (category === "Currency") {
        const assetSymbols = (market as CurrencyMarket).assetSymbol.split("/");
        ...
        marketTitle = `Will 1 ${marketSymbol} ${operatorToSymbol(
          (market as CurrencyMarket).operator
        )} ${Number((market as CurrencyMarket).targetPrice) / 10 ** 8} ${quoteSymbol}?`;
      } else {
  ...
  // Create a market object for MarketBuyInterface
  const marketForBuyInterface = market && {
    optionA: category === "Currency" ? "Yes" : (market as GeneralMarket).optionA,
    optionB: category === "Currency" ? "No" : (market as GeneralMarket).optionB,
    question: category === "Currency" 
      ? `Will 1 ${(market as CurrencyMarket).assetSymbol.split("/")[0].toUpperCase()} ${operatorToSymbol((market as CurrencyMarket).operator)} ${Number((market as CurrencyMarket).targetPrice) / 10 ** 8} ${(market as CurrencyMarket).assetSymbol.split("/")[1].toUpperCase()}?`
      : (market as GeneralMarket).question
  };

  const marketForSharesDisplay = market && {
    optionA: category === "Currency" ? "Yes" : (market as GeneralMarket).optionA,
    optionB: category === "Currency" ? "No" : (market as GeneralMarket).optionB,
    ...
    >
      {isLoadingMarketData ? (
        ...
        <>
          ...
          <CardFooter>
            {market && sharesBalance && marketForSharesDisplay && (
              <MarketSharesDisplay
                market={marketForSharesDisplay}
