...
export function MarketCard({ index, filter, category, onClick }: MarketCardProps) {
    ...
    const methodToUse = category === 'Currency' ? "function getMarketInfo(uint256 _marketId) view returns (string assetSymbol, uint8 operator, uint256 targetPrice, uint256 endTime,uint256 duration, uint8 outcome, uint256 totalOptionAShares, uint256 totalOptionBShares, bool resolved)" 
    ...
    const title = category === 'Currency' 
        ? `1 ${marketSymbol} ${operatorToSymbol(market?.operator)} ${Math.floor(parseInt(toEther(market?.targetPrice)))} ${quoteSymbol}?` 
        : market?.question;

    return (
        <Card key={index} className="flex flex-col transition-colors duration-300 ease-in-out cursor-pointer hover:bg-gray-200" onClick={() => onClick(index, title)}>
            {isLoadingMarketData ? (
                <MarketCardSkeleton />
            ) : (
                <>
                    <CardHeader>
                        {market && <MarketTime endTime={market.endTime} category={category} isResolved={isResolved}/>}
                        <CardTitle>
                            {title}
                        </CardTitle>
                    </CardHeader>
