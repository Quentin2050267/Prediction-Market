...
contract PredictionMarketNew is Ownable, ReentrancyGuard {
    ...
    function buySharesQuadratic(uint256 _marketId, bool _isOptionA, uint256 _tokensAmount) external {
        Market storage market = markets[_marketId];
        ...
        // Calculate the cost using AMM for these shares
        uint256 total_cost = AMM.getAmount(
            market.totalOptionAShares, market.totalOptionBShares, _isOptionA, shares
        );
        ...
        emit SharesPurchased(_marketId, msg.sender, _isOptionA, _tokensAmount, shares);
    }
    ...
    function buyByShares(uint256 _marketId, bool _isOptionA, uint256 _shares) external {
        ...
        emit SharesPurchased(_marketId, msg.sender, _isOptionA, _shares, _shares);
    }
    ...
    function buyShares(
        ...
        emit SharesPurchased(_marketId, msg.sender, _isOptionA, _share, _share);
    }
